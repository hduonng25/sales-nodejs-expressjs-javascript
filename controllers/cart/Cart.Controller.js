    ...
    const resuilt = await cart.aggregate([
        ...
        {$match: {"cart_details.id": id_cart_details}},
        {
            $set: {
                ...
                "cart_details.cart_details_money": {$multiply: ["$cart_details.price", quantity]}
            }
        }
    ]);
...
export async function deletedCartDetails(id_cart_details) {
    await cart.updateOne({"cart_details.id": id_cart_details}, {$set: {"cart_details.$.is_deleted": true}});
    return success("done");
};
