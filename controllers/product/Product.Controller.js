...
export async function createProduct(price, name, note, quantity, color_id, size_id, metarial_id, designs_id) {
...
export async function updateProductDetails(product_id, product_details_id, quantity, status, is_deleted) {
    const filter = [
        {$match: {id: product_id, 'product_details.id': product_details_id, is_deleted: false}},
    ...
    const update = await product.findOneAndUpdate(
        {id: product_id},
        {$set: {product_details: product_update[0].product_details}}
    );
    return success(update);
};

export async function saveImage(product_id, product_details_id, image) {
    const products = await product.findOne({id: product_id});
