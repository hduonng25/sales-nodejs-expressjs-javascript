...
export async function addProductToBill(bill_id, product_id, color_id, size_id, quantity) {
    const products = await product.findOne({id: product_id});
    const product_details = products.product_details.find((details) => details.size.id === size_id && details.color.id === color_id);
    if (product_details) {
        const bill = await bills.findOne({id: bill_id});
        const money_bill_details = products.price * quantity

        const checkProductOfBill = bill.bill_details.find((details) => details.product_details_id === product_details.id);
...
export async function editQuantityBillDetails(bill_id, bill_details_id, quantity) {
    const bill = await bills.findOne({"bill_details.id": bill_details_id});
    const result = await bills.aggregate([{
        $match: {
            id: bill_id, "bill_details.id": bill_details_id
        }
    }, {$unwind: "$bill_details"}, {$match: {"bill_details.id": bill_details_id}}, {
        $set: {
            "bill_details.quantity": quantity,
            "bill_details.money_bill_details": {$multiply: ["$bill_details.price", quantity]}
        }
    },]);

    await product.updateOne({"product_details.id": result[0].bill_details.product_details_id}, {$inc: {"product_details.$.quantity": bill.bill_details[0].quantity - quantity}});
...
export async function deletedBillDetails(bill_details_id) {
    await bills.updateOne({"bill_details.id": bill_details_id}, {$set: {"bill_details.$.is_deleted": false}});
