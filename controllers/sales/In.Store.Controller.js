...
export async function createBill(id) {
    const idBig = await bills.findOne(
        {},
        { id: 1 },
        { sort: { id: -1 } },
    );
...
export async function addProductToBill(
    ...
    if (product_details) {
        ...
        const checkProductOfBill = bill.bill_details.find(
            (details) =>
                details.product_details_id === product_details.id,
        );
        if (checkProductOfBill) {
            checkProductOfBill.quantity += quantity;
            checkProductOfBill.money_bill_details +=
                money_bill_details;
        } else {
...
export async function editQuantityBillDetails(
    ...
    await product.updateOne(
        {
            'product_details.id':
                result[0].bill_details.product_details_id,
        },
        {
            $inc: {
