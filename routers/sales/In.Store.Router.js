...
router.put(
  "/in-store/edit-quantity",
  check_edit_quantity,
  async (request, response, next) => {
    const { bill_id, bill_details_id, quantity } = request.body;
    const update = await editQuantityBillDetails(
      bill_id,
      bill_details_id,
      quantity
    );
